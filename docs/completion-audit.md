...
| 11 | Git and external-change discipline | proven through corrective freeze | The initial v2 freeze was pushed before any v2 result. A partial provision created only the dedicated uniform-access bucket, then stopped on a gcloud flag incompatibility before identities, IAM, uploads, deployment, or model calls. The correction is revalidated for a new pre-run freeze.
